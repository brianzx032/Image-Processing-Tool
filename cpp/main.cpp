...
CvImage img_orig=cv2ip.ImRead(orig_name);
CvImage img_ref=cv2ip.ImRead(ref_name);
CvImage img_eq, img_match, img_yuv, img_hsv, img_ref_hsv, img_ref_yuv;
...
// void disp_img_init(imgWidget img_widg, int hpos, int vpos)
// {
//   img_widg.image=gtk_image_new_from_pixbuf(gdk_pixbuf_copy(img_widg.pixbuf));
//   gtk_grid_attach (GTK_GRID (grid), img_widg.image, hpos, vpos, 1, 2);

//   img_widg.pdf_img=gtk_image_new_from_pixbuf(gdk_pixbuf_copy(img_widg.pdf_pix));
//   gtk_grid_attach (GTK_GRID (grid), img_widg.pdf_img, hpos+1, vpos, 1, 1);

//   img_widg.cdf_img=gtk_image_new_from_pixbuf(gdk_pixbuf_copy(img_widg.cdf_pix));
//   gtk_grid_attach (GTK_GRID (grid), img_widg.cdf_img, hpos+1, vpos+1, 1, 1);
// }
...
void all_channel_eq (GtkWidget *widget, gpointer data)
{
    switch (eq_type)
    {
    case EQ_RGB_TOG:
    ...
    eq_widg.pixbuf = gdk_pixbuf_new_from_file_at_size(comb_char(img_dir,eq_name),400,400,err);
...
void all_channel_match (GtkWidget *widget, gpointer data)
{
    switch (match_type)
    {
    case MAT_RGB_TOG:
    ...
    match_widg.pixbuf = gdk_pixbuf_new_from_file_at_size(comb_char(img_dir,match_name),400,400,err);
                            ...
                            GtkWidget *entry )
{
  ...
  orig_widg.pixbuf = gdk_pixbuf_new_from_file_at_size(orig_name,400,400,err);
                            ...
                            GtkWidget *entry )
{
  ...
  ref_widg.pixbuf = gdk_pixbuf_new_from_file_at_size(ref_name,400,400,err);
  gtk_image_set_from_pixbuf(GTK_IMAGE(ref_widg.image),ref_widg.pixbuf);
  cv2ip.ShowCDF(img_orig,ref_widg.pdf_cvImg,ref_widg.cdf_cvImg);
...
int main (int   argc, char *argv[])
{
  ...
  g_signal_connect (button_eq, "clicked", G_CALLBACK (all_channel_eq), NULL);
  ...
  g_signal_connect (button_match, "clicked", G_CALLBACK (all_channel_match), NULL);
  ...
  gtk_grid_attach (GTK_GRID (grid), orig_label, 0, 0, 2, 1);
  ...
  g_signal_connect (orig_entry, "activate",
        ...
        orig_entry);
  gtk_grid_attach (GTK_GRID (grid), orig_entry, 2, 0, 4, 1);

  ref_label = gtk_label_new ("參考圖檔：");
  gtk_grid_attach (GTK_GRID (grid), ref_label, 0, 3, 2, 1);
  ...
  g_signal_connect (ref_entry, "activate",
        ...
        ref_entry);
  gtk_grid_attach (GTK_GRID (grid), ref_entry, 2, 3, 4, 1);
